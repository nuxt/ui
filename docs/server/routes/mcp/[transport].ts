...
const handler = createMcpHandler(
  ...
  // RESOURCES
    ...
    server.tool(
      'get_component',
      'Retrieves Nuxt UI component source code with optional metadata. Parameters: componentName (string, required) - the component name in PascalCase, includeMetadata (boolean, optional) - include props/slots/events metadata. Returns: A JSON object containing name, title, description, category, documentation, documentation_url, and optionally metadata with props, slots, and emits arrays.',
      {
        componentName: z.string().describe('The name of the component (PascalCase)'),
        includeMetadata: z.boolean().optional().describe('Include component metadata (props, slots, events)')
      },
      async (params) => {
    ...
    server.tool(
      ...
      async (params) => {
        const result = await $fetch('/api/mcp/get-component', { query: { componentName: params.componentName, includeMetadata: true } })
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] }
      }
    )
    ...
    server.tool(
      ...
      async (params) => {
        const result = await $fetch('/api/mcp/get-content-page', { query: params })
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] }
      }
    )
