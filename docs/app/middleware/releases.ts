...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  if (!releases.value.length || !known) {
    try {
      ...
      // the route serves every release, the docs list the stable v5 and v4 lines
      releases.value = list.filter(release => DOCUMENTED.test(release.tag)).sort(compareReleases)
    } catch (error) {
