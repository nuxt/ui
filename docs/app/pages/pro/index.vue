...
<template>
  <div class="relative">
    ...
    >
      <template #title>
        <MDC :value="page.hero.title" unwrap="p" cache-key="pro-hero-title" />
      </template>
      <template #description>
        <MDC :value="page.hero.description" unwrap="p" cache-key="pro-hero-description" />
      </template>
      <template #top>
        <StarsBg />
      </template>
    ...
    >
      <template #description>
        <Motion :initial="{ opacity: 0, transform: 'translateY(10px)' }" :in-view="{ opacity: 1, transform: 'translateY(0)' }" :in-view-options="{ once: true }" :transition="{ delay: 0.2 }">
          <MDC :value="page.testimonial.quote" unwrap="p" class="before:content-[open-quote] after:content-[close-quote]" cache-key="pro-testimonial-quote" />
        </Motion>
      </template>
    ...
    >
      <template #title>
        <MDC :value="page.mainSection.title" unwrap="p" cache-key="pro-main-section-title" />
      </template>
      <template #description>
        <MDC :value="page.mainSection.description" unwrap="p" cache-key="pro-main-section-description" />
      </template>
    </UPageCTA>
    <UPageSection
