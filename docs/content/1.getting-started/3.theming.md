...
- skip config and accept the defaults, which are `green` and `grey`
- choose other configured Tailwind color, e.g. `rose` and `neutral`
- choose nested colors, e.g. `brand.primary` and `brand.secondary`
