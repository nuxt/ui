...
Components that have a `color` prop like [Avatar](/elements/avatar#chip), [Badge](/elements/badge#style), [Button](/elements/button#style), [Input](/elements/input#style) (inherited in [Select](/forms/select) and [SelectMenu](/forms/select-menu)) and [Notification](/overlays/notification#timeout) will use the `primary` color by default but will handle all the colors defined in your `tailwind.config.ts` or the default Tailwind CSS colors.

To achieve this, the module uses the [Tailwind CSS safelist](https://tailwindcss.com/docs/content-configuration#safelisting-classes) feature. All the classes needed by those components are safelisted for each color by default. This can lead to a lot of unused CSS classes in your final CSS file. To avoid this, you can choose which colors you want to safelist in your `nuxt.config.ts`.
...
This will allow you to use the `red` and `green` colors only for the previous components. You can also choose to safelist colors for specific components.
...
export default defineNuxtConfig({
  ui: {
    safelistColors: {
      ...
      badge: ['cyan', 'teal', 'indigo'],
      button: ['red', 'green'],
      input: [],
      notification: ['red', 'green']
    }
  }
})
```
...
::alert{icon="i-heroicons-light-bulb"}
The `primary` color is always safelisted for all the components as it uses CSS variables.
::
