...
If your theme already defines `primary` and `gray` colors, they will be converted to this new alpha-enabled version. Before Nuxt UI 2.19.0 the keys `primary` and `gray` were overwritten by Nuxt UI, but you are now free to uses these values in your Tailwind config
::
