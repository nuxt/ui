...
In the previous section, we've seen how to configure our Tailwind CSS colors using the `@theme` directive. Let's have a look at how Nuxt UI deals with colors.
...
::tip
We recommend using these colors in your application whenever possible with classes like `text-[--ui-primary]`, `border-[--ui-border]` or `bg-[--ui-bg]` for example.
::
...
::note
You can try this out by using the :prose-icon{name="i-heroicons-swatch-20-solid" class="text-[--ui-primary]"} menu in the header of this documentation.
::
...
The theme of Nuxt UI uses some CSS variables to style the components.
...
You can easily override these variables in your `app.vue`{lang="ts-type"} or in your [CSS](https://nuxt.com/docs/getting-started/styling#the-css-property):
...
Nuxt UI also automatically applies a text and background color on the `<body>` element of your app:
