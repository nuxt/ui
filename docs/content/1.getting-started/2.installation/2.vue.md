...
::note{to="#inertia"}
You can omit the `vue-router` setup when using [Inertia.js](https://inertiajs.com/).
::
...
Enable the `inertia` option to ensure compatibility with [Inertia.js](https://inertiajs.com/).
...
Note that `vue-router` is not required when using this option.
