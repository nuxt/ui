...
2. `UNotifications` component is no longer available and needed due to the usage of `UApp`:
...
3. `USlideover` now accepts `v-model:open` instead of `v-model`. Also, the content inside needs to be wrapped inside the `template #body`:
...
::note
This change affects the following components: `Modal`, `Popover`, `DropdownMenu`, `InputMenu`, `Select`, `SelectMenu`, `Tooltip`, `Collapsible`, `Drawer`.
::

4. `Toast` does not have a `timeout` property and instead it is named `duration`:

```diff
<template>
const toast = useToast();

- toast.add({ title: 'test', timeout: 3000 })
+ toast.add({ title: 'test', duration: 3000 })
```
