...
1. The `Toast` component `timeout` prop has been renamed to `duration`:
...
2. We have remove `useModal`, and `useSlideover` composables in favor of a more generic `useOverlay` composable:
...
- Modals can no longer be closed using `modal.close()`, rather, they close when a `closed` event is fired from the opened component.
- Modals can return a value by emitting the `closed` event with the desired value.
...
// First we create the modal instance
