...
Use the `loading` prop to display a loading state. it's used for async data fetching.

You can pass an `object` through the `loading` prop or globally through `ui.table.default.loadingState`.
