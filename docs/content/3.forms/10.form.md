...
Use the Form component to validate form data using schema libraries such as [Yup](https://github.com/jquense/yup), [Zod](https://github.com/colinhacks/zod), [Joi](https://github.com/hapijs/joi), or your own validation logic. It works seamlessly with FormGroups to automatically display error messages around form elements.
...
- `validate` - a function that takes the form's state as input and returns an array of `FormError` objects.
- `FormError` - an object containing a `message` field with the error message to display and a `path` field matching a FormGroup's `name`.
