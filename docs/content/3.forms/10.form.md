...
::component-example
#default
:form-example-focus{class="space-y-4 w-60"}
...
async function onError (
  event: FormSubmitEvent<{ errors: FormError<keyof Schema>[],
  ids: Record<keyof Schema, string>}>
) {
  const { path } = event.data.errors[0]
  const element = document.getElementById(event.data.ids[path])
  element.focus()
