...
::component-example
#default
:select-menu-example-objects-value-attribute{class="max-w-[12rem] w-full"}
#code
```vue
<script setup>
const people = [{
...
When NOT using the `multiple` prop while utilizing the `value-attribute` and the `option-attribute` ones, you might need to access the currently selected object and manually utilize the `label` slot to display it.
::
