...
To make development easier, we've created a CLI that you can use to generate components and locales. You can access it using the `pnpm cli make` command.
...
pnpm cli make component my-component

# Create a prose component
pnpm cli make component heading --prose

# Create a content component
pnpm cli make component block --content

# Generate only documentation template
pnpm cli make component my-component --template=docs
```
...
```bash
pnpm cli make locale --code <code>
```
...
```bash
git clone -b v5 https://github.com/nuxt/ui.git
```
...
- Follow along the [instructions](https://github.com/nuxt/ui/blob/v5/.github/PULL_REQUEST_TEMPLATE.md?plain=1) provided when creating a PR
