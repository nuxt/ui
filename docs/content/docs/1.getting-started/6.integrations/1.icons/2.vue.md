...
To bundle other icons, such as ones from other collections or used elsewhere in your app, add them to the `icon.clientBundle.icons` option in your `vite.config.ts`:
...
::tip{to="/docs/components/theme#icons"}
To change icons in part of your app only, wrap it in a Theme component with the `icons` prop.
::

::note
Icons you override in your `vite.config.ts` are embedded in your build like Nuxt UI's own, as long as they come from the `lucide` collection. Icons from other collections, and the ones you set with the Theme component's `icons` prop, are loaded on demand at runtime. To bundle them too, add them to [`icon.clientBundle.icons`](#iconify-dataset), or enable `scan`, which finds the names written in your `.vue` files but skips `vite.config.ts`.
::
