...
::note
Icons you override in `app.config.ts` or with the Theme component's `icons` prop are loaded on demand at runtime rather than embedded in the client bundle. To bundle them too, add them in `{collection}:{name}` form (for example `lucide:rocket`) to `@nuxt/icon`'s [`clientBundle.icons`](https://github.com/nuxt/icon?tab=readme-ov-file#client-bundle) in your `nuxt.config.ts`, or enable [`clientBundle.scan`](#iconify-dataset). Scan finds the names written in your `.vue` files, but it skips `app.config.ts` unless you add `ts` to its `globInclude`.
::
