...
::note
Icons you override in `app.config.ts` are fetched from the Iconify API at runtime rather than embedded in the build. To bundle them too, add them to `@nuxt/icon`'s [`clientBundle.icons`](https://github.com/nuxt/icon?tab=readme-ov-file#client-bundle) in your `nuxt.config.ts`.
::
