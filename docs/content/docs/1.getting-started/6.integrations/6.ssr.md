...
Only Nuxt UI's own icons and the ones you list in `clientBundle.icons` are bundled. Any other icon, including icons you use elsewhere in your app or icons whose collection is not installed, still loads from the Iconify API at runtime. To opt out of bundling entirely, set `icon.clientBundle` to `false`.
