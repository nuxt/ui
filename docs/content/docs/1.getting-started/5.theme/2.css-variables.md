...
Each one reads a `--ui-accent-<role>` variable before falling back to the value above (`--ui-accent-soft`, `--ui-accent-foreground`…). A color points these at its own `--ui-<color>-<role>` variables, so you can change one role of one color in your CSS and every component follows. For example, dark text on a yellow `warning`:
...
Every color utility accepts them (`text-accent`, `border-accent-border`, `divide-accent`). Use them in your own markup and set `--ui-accent` to pick the color:
...
The same variable recolors a component outside its `color` prop. Set it through `class`, and it reaches every part of the component: `<UButton class="[--ui-accent:var(--color-indigo-500)]" />`, `<UInput highlight class="[--ui-accent:var(--color-indigo-500)]" />`. Components nested inside keep their own color, like the buttons of an InputNumber.

`neutral` is a color like the others: `--ui-neutral` is `--ui-color-neutral-900` in light mode and white in dark mode, and `color="neutral"` uses the same classes as every other color.
