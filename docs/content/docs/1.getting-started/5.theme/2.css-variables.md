...
`neutral` is a color like the others. `--ui-neutral` is shade 900 in light mode and white in dark mode, like `--ui-bg-inverted`, and `color="neutral"` uses the same classes as every other color, except in Callout, inline Code, Calendar and ChatMessage.
