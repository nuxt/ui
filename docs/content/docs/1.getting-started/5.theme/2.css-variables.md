...
::tip
For a black or white primary that stays coherent across every variant, set `primary: 'neutral'` in your [**config**](/docs/getting-started/theme/design-system#runtime-configuration). It renders components with the `neutral` variant and follows your `neutral` color, so it's `neutral-900` / white rather than pure black.
::
