...
The `--ui-ease-out` curve drives the enter and exit transitions of components like [Modal](/docs/components/modal), [Popover](/docs/components/popover) or [DropdownMenu](/docs/components/dropdown-menu), while `--ui-ease-drawer` is used by the [Slideover](/docs/components/slideover) for its sliding motion.
