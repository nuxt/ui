...
## Easing :badge{label="Soon" class="align-text-top"}

Nuxt UI uses CSS variables to define the easing curves of the transitions and animations applied across components, allowing you to tune the feel of the motion consistently throughout your application.

```css
:root {
  --ui-ease-out: cubic-bezier(0.23, 1, 0.32, 1);
  --ui-ease-fluid: cubic-bezier(0.32, 0.72, 0, 1);
}
```

The `--ui-ease-out` curve drives the enter and exit transitions of components like [Modal](/docs/components/modal), [Popover](/docs/components/popover) or [DropdownMenu](/docs/components/dropdown-menu), while `--ui-ease-fluid` is used by the [Slideover](/docs/components/slideover) for its sliding motion.

::tip
You can customize these values in your `main.css` file:
...
::note
When the user enables `prefers-reduced-motion`, overlays fade in and out without the scale or slide movement.
::
