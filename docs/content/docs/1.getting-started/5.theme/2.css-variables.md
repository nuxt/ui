...
::tip
For a black or white primary that stays coherent across every variant, set `primary: 'neutral'` in your [**config**](/docs/getting-started/theme/design-system#runtime-configuration). The primary scale then mirrors your `neutral` color and `--ui-primary` points to `--ui-bg-inverted`, so it resolves to `neutral-900` in light mode and white in dark mode.
::
