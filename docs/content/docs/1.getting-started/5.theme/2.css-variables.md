...
Each one reads a `--ui-accent-<role>` variable before falling back to the value above (`--ui-accent-soft`, `--ui-accent-foreground`…). `neutral` sets them to the surface tokens (`bg-elevated`, `text-default`, `ring-accented`…), which is how a neutral component keeps its look.
...
The same variable recolors a component outside its `color` prop. It has to be set on the element that carries the color, the root of a Button or a Badge, a slot through `ui` otherwise: `<UButton class="[--ui-accent:var(--color-indigo-500)]" />`, `<UInput highlight :ui="{ base: '[--ui-accent:var(--color-indigo-500)]' }" />`.
