...
The `color` prop accepts the seven aliases above. To use another color, declare it in your `main.css` next to the others, and bridge it to Tailwind so it gets utilities like `bg-tertiary`:
...
Components read their color from the `--ui-accent` variable, so point it at your color to use it on one of them. Set it on the root of a Button or a Badge, on a slot through `ui` otherwise:

```vue
<template>
  <UButton class="[--ui-accent:var(--ui-tertiary)]" label="Special action" />
  <UInput highlight :ui="{ base: '[--ui-accent:var(--ui-tertiary)]' }" />
</template>
```
