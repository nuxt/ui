...
::tip
`neutral` is a color in its own right, not just a palette name. Assign it to any alias (e.g. `primary: 'neutral'`) and that alias renders exactly like `color="neutral"` does, using the `bg-inverted` / `bg-elevated` tokens and following whatever you set `neutral` to. This is what you want for a monochrome primary. It's distinct from assigning a specific grey palette like `primary: 'zinc'`, which gives a mid-tone zinc accent rather than the neutral treatment.
::
