...
Use the `--ease-*` theme variables to [customize the easing](https://tailwindcss.com/docs/transition-timing-function#customizing-your-theme) of the transitions and animations in your project.
...
Nuxt UI runs every enter and exit animation on `--ease-out`, and the `swing` and `elastic` indeterminate animations of [Progress](/docs/components/progress) and [Table](/docs/components/table) on `--ease-in-out`, so overriding these two variables retimes the whole library. The `carousel` animations stay on `linear` since they wrap in one direction and any curve would stall them at the loop seam.

::note
When `prefers-reduced-motion` is set, overlays fade in place instead of scaling or sliding and movement transitions are disabled. Height reveals, swipe dismissals and decorative loops are left alone since they either follow the pointer or carry meaning on their own.
::
