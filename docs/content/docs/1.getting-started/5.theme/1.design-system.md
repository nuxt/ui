...
::tip
You can also assign `neutral` itself to any alias (e.g. `primary: 'neutral'`) to get a monochrome primary. The alias then mirrors your `neutral` palette and its `--ui-primary` variable points to the `bg-inverted` token, so solid components render black on white in light mode and white on black in dark mode. This is different from assigning a specific gray palette like `primary: 'zinc'`, which gives a mid-tone zinc accent.
::
