...
Nuxt UI runs every enter and exit animation on `--ease-out`, and the indeterminate [Progress](/docs/components/progress) and [Table](/docs/components/table) loops on `--ease-in-out`, so overriding these two variables retimes the whole library.
