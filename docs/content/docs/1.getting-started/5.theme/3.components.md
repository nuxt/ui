...
Some components are composed of a single root element. In this case, the theme only defines the `base` slot like the [Container](/docs/components/container) component for example:
...
::note
These components have a [`ui` prop](#ui-prop) like any other, with a single `base` key. [Icon](/docs/components/icon) is the exception: it has no `ui` prop, so use the [`class` prop](#class-prop) to override its styles.
::
