...
By default, these classes are merged onto the component's defaults. You can also set a slot to a function to replace them instead. It receives the slot's default classes as its argument, so you can reuse part of them if needed. The classes coming from `variants` and `compoundVariants` still apply on top, exactly like when you pass a string.
...
::note
The `ui` prop is resolved after the `variants`, so unlike in the global config, a function here replaces the slot's **resolved** classes. Its argument contains everything the `variants` and `compoundVariants` matched too. Plain classes passed alongside, like a `class` prop targeting the same slot, still merge on top of the returned ones.
::
