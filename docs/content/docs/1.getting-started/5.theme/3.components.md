...
::tip{icon="i-lucide-scan-eye"}
**Interactive Slot Explorer**: Every component example in the documentation has a **Slots** button in the top-right corner. Click it to see all available theme slots and hover over a slot name to highlight the corresponding element in the component preview. This is a great way to understand how slots map to the rendered output.
::
