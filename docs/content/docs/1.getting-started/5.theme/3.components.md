...
The classes you set on a slot are merged after everything the theme resolved for it, its `variants` and `compoundVariants` included, so they win any conflict the same way the [`ui` prop](#ui-prop) does. Your own `compoundVariants` come last, since they are the more specific of the two. Your `variants` are merged into the theme's and stay beneath its `compoundVariants`, which are often the exception to a variant, like a `square` button ignoring the padding of its `size`.

You can also set a slot to a function to replace the slot's default classes instead of merging onto them. It receives those classes as its argument, so you can reuse part of them. Classes from `variants` and `compoundVariants` still apply on top.
