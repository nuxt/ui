...
::note
The `ui` prop is resolved after the `variants`, so unlike in the global config, a function here replaces the slot's **resolved** classes. Its argument contains everything the `variants` and `compoundVariants` matched too, and the returned classes are the only ones left on the slot.
::
