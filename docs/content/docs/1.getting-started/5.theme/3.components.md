...
By default, these classes are merged onto the component's defaults. You can also set a slot to a function to replace its classes entirely. It receives the resolved default classes as its argument, so you can reuse part of them if needed.
...
A slot value can also be a function to replace its classes instead of merging them, the same as in the global config:
