...
These classes are **merged** onto the component's defaults. To **replace** a slot's classes entirely, set it to a function returning the new classes — it receives the slot's resolved default classes as its argument, so you can reuse part of them if needed:
...
To strip the default classes from **all** components at once (to bring your own design system or reduce HTML output), use the build-time `theme.unstyled` option.
::
...
Like the global config, a slot can be a **function** to **replace** its classes instead of merging them, receiving the slot's resolved default classes as its argument:
