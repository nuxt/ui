...
#### Install Nuxt UI and Unhead :badge{label="Soon" class="align-text-top"}

::code-group{sync="pm"}

```bash [pnpm]
pnpm add @nuxt/ui @unhead/vue tailwindcss
```

```bash [yarn]
yarn add @nuxt/ui @unhead/vue tailwindcss
```

```bash [npm]
npm install @nuxt/ui @unhead/vue tailwindcss
```

```bash [bun]
bun add @nuxt/ui @unhead/vue tailwindcss
```
