...
Use the `prose` option to enable Nuxt UI `<Prose>` components and their theme. This is useful when rendering Markdown content with libraries like [comark](https://github.com/comarkdown/comark).
...
Use the `content` option to enable both Nuxt UI `<Prose>` and `<UContent>` components along with their themes.
...
::note
When disabled, you can still import components explicitly from `@nuxt/ui/components` and Prose components from `@nuxt/ui/components/prose`.
::
