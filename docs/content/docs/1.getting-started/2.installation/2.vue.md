...
| `function`{lang="ts-type"} | Custom navigation handler for frameworks like Hybridly. |
...
::tip
You can pass a function to provide custom navigation logic for frameworks like **Hybridly**:
...
::note
When set to `false`, `'inertia'` or a function, `vue-router` is not required as a dependency.
::
