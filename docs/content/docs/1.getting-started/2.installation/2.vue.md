...
To compile the same theme / `app.config.ts` class strings with StyleX instead of Tailwind CSS:

::code-group{sync="pm"}

```bash [pnpm]
pnpm add @nuxt/ui @stylexjs/stylex
```

```bash [yarn]
yarn add @nuxt/ui @stylexjs/stylex
```

```bash [npm]
npm install @nuxt/ui @stylexjs/stylex
```

```bash [bun]
bun add @nuxt/ui @stylexjs/stylex
```
...
@import "@nuxt/ui/stylex";
```

Theme slots and `app.config.ts` stay written as utility class strings; the StyleX engine converts them at build time. Put class overrides in `app.config.ts` — runtime `:ui` / `class` strings are not converted in the browser.
...
### `theme.unstyled` :badge{label="4.9+" class="align-text-top"}
...
Compile the same theme / `app.config.ts` class strings with StyleX instead of Tailwind CSS. Authoring does not change; the conversion happens at build time.
...
Install `@stylexjs/stylex` instead of `tailwindcss`. Put class overrides in `app.config.ts` — runtime `:ui` / `class` strings are not converted in the browser.
