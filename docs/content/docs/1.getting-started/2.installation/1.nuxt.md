...
Use the `theme.prefix` option to configure a prefix for all Tailwind CSS utility classes to avoid naming conflicts with other CSS frameworks or libraries.
...
You might need to set `fonts.processCSSVariables` to `true` in your `nuxt.config.ts` to use the prefix option with the `@nuxt/fonts` module.
::
