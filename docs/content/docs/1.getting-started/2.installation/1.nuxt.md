...
::card-group
  ...
  color: neutral
  ---
  A changelog template to display your repository releases notes from GitHub powered by Comark.
  ::
...
Use the `prose` option to force the import of Nuxt UI [`Prose` components](/docs/typography) even if `@nuxtjs/mdc`, `@nuxt/content` or `@comark/nuxt` is not installed.
