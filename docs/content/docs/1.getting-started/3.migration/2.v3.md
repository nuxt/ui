...
::note
You can also use the new [design tokens](/docs/getting-started/theme#neutral-palette) to handle light and dark mode:
...
::note{to="/docs/getting-started/theme#colors"}
...
Nuxt UI components are now styled using the [Tailwind Variants API](/docs/getting-started/theme#components-theme), which makes all the overrides you made using the `app.config.ts` and the `ui` prop obsolete.

- Update your [`app.config.ts`](/docs/getting-started/theme#config) to override components with their new theme:
...
- Update your [`ui` props](/docs/getting-started/theme#props) to override each component's slots using their new theme:
...
| `LandingFAQ` | Removed (use [`PageAccordion`](/docs/components/page-accordion) instead) |
