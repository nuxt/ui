...
#### Accordion
The `Accordion` component has been hugely redesigned to provide a clearer state model, improved accessibility, and more flexible customization. As a result, its API is not fully backward compatible.
...
- The `rows` prop has been renamed in favor of `data`:
...
- Slots receive a row wrapper, not the raw object, augmented with different properties:
...
- `Form` now always validates on submit, as a result the `validate-on` prop only controls which input events trigger validation. If you want the `Form` to validate only on submit, you need to pass an empty array to `validate-on`:
