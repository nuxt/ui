...
- The `SelectMenu` component `searchable` prop has been renamed to `search-input`:

```diff
<template>
- <USelectMenu searchable :items="items" />
+ <USelectMenu search-input :items="items" />
</template>
```
...
- The `Table` component slots receive a row wrapper, not the raw object, augmented with different properties:
...
- The `DropdownMenu` component items structure has changed. Items are now nested with `children` instead of a 2D array:

```diff
<script setup lang="ts">
- const items = [[{ label: 'Edit' }], [{ label: 'Delete' }]]
+ const items = [{ label: 'Edit' }, { type: 'separator' }, { label: 'Delete' }]
</script>
```
