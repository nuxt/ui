...
5. Replace `<MDC>` with `<Markdown>` for parts-based rendering. The `Markdown` component is purpose-built for streaming Markdown, it incrementally renders tokens as they arrive from the AI. Use AI SDK helpers and the `isPartStreaming` utility from `@nuxt/ui/utils/ai`.

::note
When using the `shiki` plugin, add the following CSS to your stylesheet for dark mode support:
...
::note{to="https://ai-sdk.dev/docs/migration-guides/migration-guide-7-0" target="_blank"}
For more details on the AI SDK changes, review the **official AI SDK migration guides**.
::

::tip{to="https://github.com/nuxt/ui/pull/6636" target="_blank"}
View all changes in the **AI SDK v7 upgrade PR** for a detailed migration reference.
::
