...
1. Update `@ai-sdk/vue` and `ai` dependencies in your `package.json` and add `@comark/nuxt` (replaces `@nuxtjs/mdc` for rendering AI responses as streaming Markdown):
...
5. Replace `<MDC>` with `<Comark>` for parts-based rendering. The `Comark` component from `@comark/nuxt` is purpose-built for streaming Markdown — it incrementally renders tokens as they arrive from the AI. Use AI SDK helpers and the `isPartStreaming` utility from `@nuxt/ui/utils/ai`:
