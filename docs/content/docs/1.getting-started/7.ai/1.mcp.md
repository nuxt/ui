...
- AI uses `list_component_sections` → discovers: props, slots, theme, usage, examples
- AI uses `get_component_sections` with `sections=theme,props` → fetches only relevant parts
