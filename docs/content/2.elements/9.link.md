...
It renders an `<a>` tag when a `to` prop is provided, otherwise, it defaults to rendering a `<button>` tag. The default behavior can be customized using the `as` prop.
