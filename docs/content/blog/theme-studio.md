...
The studio isn't only a page on this site. It's a layer any Nuxt UI app can extend, with a drop-in component:

```vue
<ThemeStudioButton mode="user" title="Customise Theme" @change="save" />
```

Same engine, two audiences. `mode="dev"` gives you the whole studio for theming your own app. `mode="user"` curates it down to approved presets, plain colour dropdowns and a light/dark switch - the version you hand your own users, without exposing your design system's internals. It starts from your app's existing theme rather than Nuxt UI's, and persists locally. Every edit emits the full theme document, so syncing someone's theme to their account is a few lines.
