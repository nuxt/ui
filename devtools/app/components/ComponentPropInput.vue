...
<template>
  <div v-if="schema">
    ...
    <USelectMenu v-else-if="schema?.kind === 'enum' && name === 'color'" v-model="modelValue" :items="parseEnumValues(schema.schema)" class="w-48">
      <template #item="{ item }">
        <div class="flex items-baseline gap-2">
          <span class="size-2 rounded-full" :class="`bg-${item}-500`" />
          <p>{{ item }}</p>
        </div>
      </template>
      <template #leading="{ modelValue: color }">
        <span v-if="color" class="size-2 rounded-full" :class="`bg-${color}-500`" />
      </template>
    </USelectMenu>
