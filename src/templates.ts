...
export function getTemplates(options: ModuleOptions, uiConfig: Record<string, any>, nuxt?: Nuxt, resolve?: Resolver['resolve']) {
  ...
  // Separate template for @source directives (cannot be nested in CSS layers)
  templates.push({
    filename: 'ui-sources.css',
    write: true,
    getContents: async () => {
      return await generateSources()
    }
  })

  // Separate template for theme styles (can be used with CSS cascade layers)
  templates.push({
    filename: 'ui-theme.css',
    write: true,
    getContents: async () => {
...
`
  ...
  // Combined template for backwards compatibility
  templates.push({
    filename: 'ui.css',
    write: true,
    getContents: async () => {
