...
export default (options: Required<ModuleOptions>) => ({
  ...
  variants: {
    ...
    to: {
      true: {
        ...
        image: 'transform transition-transform duration-200 ease-out group-hover/blog-post:scale-110',
        avatar: 'inline-flex transform transition-transform duration-200 ease-out hover:scale-115 rounded-full outline-primary/25 focus-visible:outline-3'
      }
    },
    image: {
      true: ''
  ...
  compoundVariants: [{
    ...
    class: {
      root: 'hover:bg-elevated/50',
      header: ['group-hover/blog-post:shadow-none', options.theme.transitions && 'transition-[box-shadow,border-radius] duration-200 ease-out']
    }
  }, {
    variant: 'ghost',
    to: true,
