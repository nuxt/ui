...
const [DefineLinkTemplate, ReuseLinkTemplate] = createReusableTemplate<{ item: NavigationMenuItem, index: number, active?: boolean, trigger?: boolean }>()
...
function onLinkTrailingClick(e: Event, item: NavigationMenuItem, trigger?: boolean) {
  ...
  if (props.orientation === 'horizontal') {
    e.preventDefault()
  } else if (props.orientation === 'vertical' && !props.collapsed && trigger) {
    e.preventDefault()
    e.stopPropagation()
  }
}
...
<template>
  <DefineLinkTemplate v-slot="{ item, active, index, trigger }">
    <slot :name="((item.slot || 'item') as keyof NavigationMenuSlots<T>)" :item="item" :index="index" :active="active" :ui="ui">
      ...
      <component
        :is="trigger && props.orientation === 'vertical' && item.children?.length && !props.collapsed ? AccordionTrigger : 'span'"
        ...
        :as="trigger && props.orientation === 'vertical' && item.children?.length && !props.collapsed ? 'span' : undefined"
        data-slot="linkTrailing"
        :class="ui.linkTrailing({ class: [props.ui?.linkTrailing, item.ui?.linkTrailing] })"
        @click="(e: Event) => onLinkTrailingClick(e, item, trigger)"
      >
  ...
  <DefineItemTemplate v-slot="{ item, index, level = 0, listIndex = 0 }">
    ...
    >
      <div v-if="props.orientation === 'vertical' && item.type === 'label' && !props.collapsed" data-slot="label" :class="ui.label({ class: [props.ui?.label, item.ui?.label, item.class] })">
        <ReuseLinkTemplate :item="item" :index="index" :trigger="true" />
      </div>
        ...
        >
          <UPopover v-if="props.orientation === 'vertical' && props.collapsed && item.children?.length && (!!props.popover || !!item.popover)" v-bind="{ ...popoverProps, ...(typeof item.popover === 'boolean' ? {} : item.popover || {}) }" :ui="{ content: ui.content({ class: [props.ui?.content, item.ui?.content] }) }">
            <ULinkBase v-bind="slotProps" data-slot="link" :class="ui.link({ class: [props.ui?.link, item.ui?.link, item.class], active: active || item.active, disabled: !!item.disabled, level: level > 0 })">
              <ReuseLinkTemplate :item="item" :active="active || item.active" :index="index" :trigger="!!(slotProps as any).href" />
            </ULinkBase>
