...
const uiTheme = useComponentUiTheme('select', () => ({ slots: props.ui }))
...
<template>
  ...
  >
    ...
    <SelectPortal v-bind="portalProps">
      <SelectContent :class="ui.content({ class: uiTheme?.slots?.content })" v-bind="contentProps">
        <slot name="content-top" />

        <div role="presentation" :class="ui.viewport({ class: uiTheme?.slots?.viewport })">
          <SelectGroup v-for="(group, groupIndex) in groups" :key="`group-${groupIndex}`" :class="ui.group({ class: uiTheme?.slots?.group })">
            <template v-for="(item, index) in group" :key="`group-${groupIndex}-${index}`">
              <SelectLabel v-if="isSelectItem(item) && item.type === 'label'" :class="ui.label({ class: [uiTheme?.slots?.label, item.ui?.label, item.class] })">
                {{ get(item, props.labelKey as string) }}
              </SelectLabel>

              <SelectSeparator v-else-if="isSelectItem(item) && item.type === 'separator'" :class="ui.separator({ class: [uiTheme?.slots?.separator, item.ui?.separator, item.class] })" />
              ...
              >
                <slot name="item" :item="(item as NestedItem<T>)" :index="index">
                  ...
                  <SelectItemText :class="ui.itemLabel({ class: [uiTheme?.slots?.itemLabel, isSelectItem(item) && item.ui?.itemLabel] })">
        ...
        <SelectArrow v-if="!!arrow" v-bind="arrowProps" :class="ui.arrow({ class: uiTheme?.slots?.arrow })" />
      </SelectContent>
    </SelectPortal>
  </SelectRoot>
</template>
