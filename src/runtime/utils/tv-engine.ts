...
import type { ClassValue, TVMergeConfig } from '../types/tv-engine'

/**
 * The variants engine, replacing `tailwind-variants` 3.2.2. It covers exactly
 * the surface our themes use — `extend`, `base`, `slots`, `variants`,
 * `compoundVariants`, `defaultVariants` — and deliberately leaves out
 * `compoundSlots`, responsive variants and the per-call config argument, none of
 * which appear in any theme or call site.
 *
 * Merge semantics mirror 3.2.2 exactly (string-concat variant merging, slot
 * joining, the `falsyToString` / `|| 'false'` variant-key lookup, nullish-or-false
 * compound matching), so resolved classes are byte-identical and the snapshot
 * suite is the spec.
 *
...
/* ------------------------------------------------------------------ *
 * class joining (`cx` semantics: clsx-style, `undefined` when there are none)
 * ------------------------------------------------------------------ */
...
/**
 * Join and merge class values under a given config, curried so the config comes
 * last: `cnMerge('px-2', 'px-4')(config)`.
 */
export function cnMerge(...classes: any[]): (config?: TVMergeConfig) => string | undefined {
  return config => mergeClasses(config, ...classes)
}
...
function resolveSpec(options: Record<string, any>, config: TVMergeConfig | undefined): Spec {
  ...
  const componentSlots: Record<string, any> = !isEmptyObject(ownSlots)
    ? { base: cx(options.base, isExtendedSlotsEmpty && extend?.base), ...ownSlots }
    : {}
  ...
  if (isExtendedSlotsEmpty) {
    ...
    slots = { ...extend.slots }
    const own = isEmptyObject(componentSlots) ? { base: options.base } : componentSlots
    for (const key in own) {
      slots[key] = key in slots ? cx(slots[key], own[key]) : own[key]
    }
  }
...
export function createTV(config?: TVMergeConfig) {
