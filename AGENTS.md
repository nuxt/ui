...
| Template slots | Every element styled by a slot carries `data-slot="<component>-<slot>"`, and the outermost one the component name alone (`data-slot="card"`, `data-slot="card-header"`). `node scripts/data-slot.mjs` writes them from the `ui.<slot>()` on each tag. Prose components emit none. |
