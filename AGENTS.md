...
| Computed ui | Always use `computed(() => tv(theme, overrides.value)(...))` for reactive theming, with `const overrides = useComponentOverrides(() => appConfig.ui?.<name>)`: it carries `<UTheme unstyled>` to the engine and types the variant values the app adds |
