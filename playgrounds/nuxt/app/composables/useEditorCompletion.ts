...
export function useEditorCompletion(editorRef: Ref<{ editor: any | undefined } | null | undefined>, options: UseEditorCompletionOptions = {}) {
  ...
  function getMarkdownBefore(editor: any, pos: number): string {
    const { state } = editor
  ...
  const handlers = {
    ...
    aiTranslate: {
      ...
      isActive: (_editor: any, cmd: { language?: string } | undefined) => !!(isLoading.value && mode.value === 'translate' && language.value === cmd?.language),
      isDisabled: (editor: any) => editor.state.selection.empty || !!isLoading.value
    }
  }
