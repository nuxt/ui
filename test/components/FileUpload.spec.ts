...
describe('FileUpload', () => {
  ...
  it('hides the icon when icon is false', async () => {
    ...
    expect(withIcon.find('[data-slot="avatar-icon"]').exists()).toBe(true)

    const withoutIcon = await mountSuspended(FileUpload, { props: { icon: false } })
    expect(withoutIcon.find('[data-slot="avatar-icon"]').exists()).toBe(false)
  })
  ...
  it('hides the icon when icon is false with variant button', async () => {
    ...
    expect(withIcon.find('[data-slot="file-upload-icon"]').exists()).toBe(true)

    const withoutIcon = await mountSuspended(FileUpload, { props: { variant: 'button', icon: false } })
    expect(withoutIcon.find('[data-slot="file-upload-icon"]').exists()).toBe(false)
  })
