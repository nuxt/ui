...
test('factory build', async ({ bench }) => {
  await bench.compare(
...
test('invocation (prebuilt factory)', async ({ bench }) => {
...
test('build + invoke (current fused pattern)', async ({ bench }) => {
  await bench.compare(
    bench('button', () => {
      for (let i = 0; i < ITERATIONS; i++) {
...
test('slot invocation', async ({ bench }) => {
